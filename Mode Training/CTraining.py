...
def fetch_data(symbol='NIFTY 50', start_date=None, end_date=None, save_path='nifty_data.csv'):
    """
    Fetch historical OHLCV data for NIFTY 50 from NSE
    
    WHY: We need historical price data to train our model.
    
    NOTE: nsepy has compatibility issues. This creates synthetic data.
    For production, replace with real 5-min data from:
    - Broker API (Zerodha Kite, Upstox, etc.)
    - Manual NSE downloads
    - Paid data vendor
    """
    ...
    print(f"Creating synthetic data from {start_date.date()} to {end_date.date()}...")
    print("PRODUCTION WARNING: Replace this with real NSE 5-minute data!")
    
    # Create 5-minute intervals for market hours (9:15 AM to 3:30 PM IST)
    date_range = pd.date_range(start=start_date, end=end_date, freq='5T')
    
    # Filter to market hours
    date_range = date_range[
        ((date_range.hour == 9) & (date_range.minute >= 15)) |
        ((date_range.hour > 9) & (date_range.hour < 15)) |
        ((date_range.hour == 15) & (date_range.minute <= 30))
    ]
    ...
    returns = np.random.normal(0.00005, 0.008, n)
    
    # Add volatility clustering (GARCH-like behavior)
    volatility = np.ones(n) * 0.008
    for i in range(1, n):
        volatility[i] = 0.7 * volatility[i-1] + 0.3 * abs(returns[i-1])
        returns[i] = np.random.normal(0.00005, volatility[i])
    
    close_prices = base_price * np.exp(np.cumsum(returns))
    
    # Generate OHLC from close with realistic relationships
    high_noise = np.abs(np.random.normal(0, 0.003, n))
    low_noise = np.abs(np.random.normal(0, 0.003, n))
    
    high_prices = close_prices * (1 + high_noise)
    low_prices = close_prices * (1 - low_noise)
    
    # Open is previous close with small gap
    open_prices = np.roll(close_prices, 1) * (1 + np.random.normal(0, 0.001, n))
    ...
    # Volume with realistic intraday pattern
    # Higher volume at open and close
    hour_minute = date_range.hour + date_range.minute / 60
    volume_pattern = 1 + 0.5 * np.exp(-((hour_minute - 9.25)**2) / 2) + 0.3 * np.exp(-((hour_minute - 15.25)**2) / 2)
    base_volume = np.random.lognormal(15, 0.5, n)
...
def evaluate_model(y_true, y_pred, y_pred_proba, model_name):
    ...
    print(f"Actual Neg      {cm[0][0]:<16} {cm[0][1]:<16}")
    print(f"Actual Pos      {cm[1][0]:<16} {cm[1][1]:<16}")
...
def main():
    ...
    print("1. This model uses SYNTHETIC data. Replace with real 5-min NSE data.")
